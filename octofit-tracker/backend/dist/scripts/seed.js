...
async function seedDatabase() {
    ...
    for (const collection of collections) {
        await mongoose_1.default.connection.db?.dropCollection(collection).catch(() => undefined);
    }
    ...
    await User_1.default.updateMany({ _id: { $in: [users[0]._id, users[1]._id] } }, { $set: { teamId: teamOne._id } });
    await User_1.default.updateMany({ _id: { $in: [users[2]._id, users[3]._id] } }, { $set: { teamId: teamTwo._id } });
